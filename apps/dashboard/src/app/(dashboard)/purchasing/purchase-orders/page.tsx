...
import { FileSpreadsheet, Plus, Search, Trash2, Search as SearchIcon } from 'lucide-react';
...
export default function PurchaseOrdersPage() {
  ...
  const handleRemoveItem = (productId: string) => setItems(items.filter((i) => i.productId !== productId));

  const handleItemChange = (productId: string, field: string, value: string) => {
    setItems(items.map((i) => (i.productId === productId ? { ...i, [field]: Number(value) } : i)));
  };

  const handleSubmit = async () => {
    if (!selectedSupplierId || items.length === 0) return alert('Select a supplier and add at least one item');
    try {
      await api.createPurchaseOrder({ supplierId: selectedSupplierId, notes, items });
      setShowModal(false);
  ...
  const filtered = pos.filter(
    (po) => po.number.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="min-h-full bg-slate-900 text-slate-200 p-8 flex flex-col h-full">
      <div className="flex justify-between items-center mb-8">
        ...
        <button onClick={() => setShowModal(true)} className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white px-6 py-2.5 rounded-lg font-semibold shadow-lg shadow-blue-500/20 transition-all flex items-center gap-2">
      ...
      <div className="mb-6 max-w-md relative">
        ...
        <input type="text" placeholder="Search by PO number or supplier..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-slate-800 border border-slate-700 focus:border-blue-500 rounded-lg pl-10 pr-4 py-2.5 text-sm text-white placeholder-slate-500 transition-colors outline-none" />
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <select value={selectedSupplierId} onChange={(e) => setSelectedSupplierId(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2.5 text-white text-sm focus:border-blue-500 outline-none">
                ...
                <div>
                  ...
                  <input type="text" value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2.5 text-white text-sm focus:border-blue-500 outline-none" placeholder="Delivery instructions, etc." />
                </div>
              </div>

              <div>
                <label className="block text-sm text-slate-300 mb-2">Add Products</label>
                <div className="relative">
                  <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 w-5 h-5" />
                  <input type="text" value={productSearch} onChange={(e) => setProductSearch(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-lg pl-10 pr-4 py-2.5 text-white text-sm focus:border-blue-500 outline-none" placeholder="Search products by name or SKU..." />
                  {products.length > 0 && (
                    ...
                    <tbody className="divide-y divide-slate-700/50">
                      {items.map((item) => (
                        <tr key={item.productId} className="bg-slate-800 text-sm">
                          ...
                          <td className="p-3"><input type="number" value={item.unitPrice} onChange={(e) => handleItemChange(item.productId, 'unitPrice', e.target.value)} className="w-full bg-slate-900 border border-slate-600 rounded px-2 py-1 outline-none focus:border-blue-500 text-white" /></td>
                          <td className="p-3"><input type="number" min="1" value={item.qty} onChange={(e) => handleItemChange(item.productId, 'qty', e.target.value)} className="w-full bg-slate-900 border border-slate-600 rounded px-2 py-1 outline-none focus:border-blue-500 text-white" /></td>
                          <td className="p-3 text-slate-300 font-medium">Rs. {(item.qty * item.unitPrice).toFixed(2)}</td>
                          <td className="p-3"><button onClick={() => handleRemoveItem(item.productId)} className="text-slate-500 hover:text-red-400"><Trash2 className="w-4 h-4" /></button></td>
                        </tr>
                      ))}
                      <tr className="bg-slate-900/30">
                        <td colSpan={3} className="p-3 text-right text-slate-400 font-medium">Grand Total:</td>
                        <td className="p-3 text-white font-bold text-lg">Rs. {items.reduce((sum, i) => sum + i.qty * i.unitPrice, 0).toFixed(2)}</td>
                        <td></td>
                      </tr>
                    </tbody>
                  </table>
            ...
            <div className="px-6 py-4 border-t border-slate-800 bg-slate-800/50 flex justify-end gap-3">
              ...
              <button onClick={handleSubmit} disabled={!selectedSupplierId || items.length === 0} className="bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white px-6 py-2 rounded-lg font-medium shadow-lg shadow-blue-500/20">
                Create PO
              </button>
            </div>
          </div>
