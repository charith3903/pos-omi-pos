...
import { ChefHat, Clock, RefreshCw, CheckCircle2, Loader2, Bell, X } from 'lucide-react';
    ...
    <div className="min-h-full bg-gray-900 text-white p-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {newAlert && (
            <div className="flex items-center gap-1 text-xs bg-red-500 px-2 py-1 rounded-full animate-pulse">
              <Bell className="w-3 h-3" /> New KOT!
            </div>
          )}
        </div>
        <div className="flex items-center gap-3">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20 text-gray-500">
          <Loader2 className="w-6 h-6 animate-spin mr-2" /> Loading kitchen…
        </div>
      ) : displayed.length === 0 ? (
                  ...
                  <div className="px-4 pb-4">
                    ...
                    >
                      {bumping === kot.id ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> :
                        meta.next === 'COOKING' ? '▶ Start Cooking' :
